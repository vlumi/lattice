...
per-platform uploads never happen by hand in Media Manager.

Reads shots/<platform>/<shot>-<platform>.png (or shots/<platform>/<lang>/… once
there is more than one language), maps platform → the ASC screenshot set
(display type), REPLACES each set's contents, and uploads in the agreed store
order (mid-game first, the dark twin last).
...
STORE_ORDER = ["mid-game", "choosing-a-line", "history", "daily", "replay",
               "new-game", "how-to-play", "nearby", "mid-game-dark"]
...
def main():
    ...
    for platform, (ver_platform, display_type) in PLATFORMS.items():
        ...
        for lang, locale in LOCALES.items():
            # Single-language today, so `make shots` writes straight into
            # shots/<platform>/. Prefer a <lang>/ subdir when one exists, so
            # adding a second language needs no change here.
            lang_dir = os.path.join(plat_dir, lang)
            if not os.path.isdir(lang_dir):
                lang_dir = plat_dir
            files = ordered_files(lang_dir, platform)
            if not files:
