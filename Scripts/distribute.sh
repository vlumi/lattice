...
case "$platform" in
    ios)   scheme="Lattice-iOS";   destination="generic/platform=iOS";   ext="ipa" ;;
    macos) scheme="Lattice-macOS"; destination="generic/platform=macOS"; ext="pkg" ;;
    *) echo "usage: distribute.sh <ios|macos> [--no-upload|--upload-only]" >&2; exit 2 ;;
esac

out="dist/${platform}"
archive="${out}/Lattice-${platform}.xcarchive"

if [ "$build" -eq 1 ]; then
    project="Lattice.xcodeproj"
