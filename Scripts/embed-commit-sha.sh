...
# identifies the exact source it came from — which build is this, exactly.
# Wired as a run-script build phase in project.yml for both app targets, so it
# fires on `xcodebuild` AND on an Xcode Organizer "Archive" (Apple sets
# INFOPLIST_PATH / TARGET_BUILD_DIR for us).
#
...
# clean commit. Copied from Donpa's Scripts/embed-commit-sha.sh.
